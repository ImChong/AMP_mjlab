...
def zero_command_stability(
  ...
  """Reward quiet standing only when the commanded velocity is near zero.

  This makes ``cmd=(0, 0, 0)`` an explicit balance sub-task instead of letting
  the policy treat standing as an infrequent corner case of velocity tracking.
  """
  asset: Entity = env.scene[asset_cfg.name]
  command = env.command_manager.get_command(command_name)
  assert command is not None, f"Command '{command_name}' not found."
...
def zero_command_action_rate_l2(
  ...
  """Penalize action changes only for near-zero commands to reduce standing jitter."""
  command = env.command_manager.get_command(command_name)
  assert command is not None, f"Command '{command_name}' not found."
